...
def sentiment_analysis(sentiment_request: SentimentRequest):
    ...
    for sentence in sentences:
        translated_sentence = translator.translate(sentence, src='es', dest='en').text
        print(translated_sentence)
        sentence_cleaned = sentence_cleaning(translated_sentence)
        polarity_scores = cal_polarity(sentence_cleaned)
...
def sentence_cleaning(sentence):
    ...
    sentence = ' '.join(
        word for word in sentence.split() if word not in stp_words)
    return sentence


def cal_polarity(sentence):
    polarity_scores = []
    for sentence in TextBlob(sentence).sentences:
