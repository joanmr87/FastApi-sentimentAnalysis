...
def fetch_emails(mail, from_timestamp, to_timestamp):
    ...
    if from_timestamp:
        ...
        from_datetime = datetime.datetime.fromisoformat(from_timestamp)

        # Convert datetime object to date object
        from_date = from_datetime.date()

        # Convert date object to IMAP-compliant date format
    ...
    if to_timestamp:
        # Convert timestamp string to datetime object
        to_datetime = datetime.datetime.fromisoformat(to_timestamp)

        # Convert datetime object to IMAP-compliant date format
        to_imap_date = to_datetime.strftime("%d-%b-%Y")
        ...
        for response in msg:
            if isinstance(response, tuple):
                ...
                if any(domain in email_message["from"] for domain in domains):
                    ...
                    email_date = date_obj.date().isoformat()

                    # Yield the email message and date
                    yield email_message, email_date
