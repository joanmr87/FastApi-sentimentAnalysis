...
def sentiment_analysis(sentiment_request: SentimentRequest):
    ...
    # Obtener las oraciones de los correos electrónicos
    user = sentiment_request.email_user
    password = sentiment_request.email_password
    mail = connect_to_mail_server('imap.gmail.com', user, password)
    emails = fetch_emails(mail, sentiment_request.from_timestamp, sentiment_request.to_timestamp)
    sentences = []
    ...
    # Limpiar los correos electrónicos y calcular la polaridad
    cleaned_sentences = []
    for sentence in sentences:
        cleaned_sentence = email_cleaning(sentence)
        cleaned_sentences.append(cleaned_sentence)
    polarities = []
    for cleaned_sentence in cleaned_sentences:
        polarity_scores = cal_polarity(cleaned_sentence)
        polarities.append(polarity_scores)

    # Devolver una lista de oraciones con su polaridad
    result = []
    for i, sentence in enumerate(sentences):
        result.append({"sentence": sentence, "polarity": polarities[i]})
