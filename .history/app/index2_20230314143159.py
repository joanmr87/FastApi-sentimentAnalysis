        ...
        for response in msg:
            if isinstance(response, tuple):
                ...
                if any(domain in email_message["from"] for domain in domains):
                    ...
                    # Compare the email date to the from_timestamp and to_timestamp arguments
                    if (not from_timestamp or email_date >= from_date) and \
                       (not to_timestamp or email_date < to_date):
                        yield email_message
   ...
   for email_message in fetch_emails(mail, sentiment_request.fromTimestamp, sentiment_request.toTimestamp):
        ...
        for email in emails:
            ...
            for polarity_score in polarity_scores:
                sentence_result = {"polarityScore": polarity_score, "topic": "HARDCODED"}
